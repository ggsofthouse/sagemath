...
class SeedGenerator:
    ...
    @staticmethod
    def generate_sha256_timestamps(year_start=2014, year_end=2015, count=100000000):
        """Gera SHA256(ts_int) e SHA256(str(ts))."""
        start_ts = int(datetime(year_start, 1, 1).timestamp())
        end_ts   = int(datetime(year_end, 12, 31, 23, 59, 59).timestamp())
        for ts in range(start_ts, min(start_ts + count, end_ts)):
            yield hashlib.sha256(ts.to_bytes(8, 'big')).digest()
            yield hashlib.sha256(str(ts).encode('utf-8')).digest()

    @staticmethod
    def generate_40_48bit(start=0, count=100000000):
        """Gera sementes inteiras de 40 a 48 bits."""
        for num in range(start, start + count):
            yield num.to_bytes(6, 'big')
    ...
    def generate_wordlist_passphrases(words, max_pass_len=4):
        ...
        for word in words:
            ...
            for i in range(100):
                yield f"{word}{i}".encode('utf-8')
                yield f"{word}_{i}".encode('utf-8')
