"""
TESTE DE LOTE GLV LLL: PUZZLES #76 A #84
Autor: Antigravity AI Engine

Este script executa a decomposição de redes GLV via LLL com Aritmética Racional Exata
para a segunda bateria de Puzzles Bitcoin não resolvidos (76 a 84).
"""
...
def carregar_puzzles_76_84() -> List[Dict]:
    with open("e:\\sagemath\\puzzles_unsolved_database.json", "r", encoding="utf-8") as f:
        data = json.load(f)
        return [p for p in data["puzzles"] if 76 <= p["number"] <= 84]
...
def executar_teste_lote_76_84():
    puzzles = carregar_puzzles_76_84()
    print("=========================================================================")
    print(f"   TESTE DE LOTE GLV LLL: PUZZLES #76 A #84 ({len(puzzles)} PUZZLES)")
    print("=========================================================================")
    ...
    for p in puzzles:
        ...
        bits = p["bits"]
        addr = p["address"]
        hex_min = int(p["hex_min"], 16)
        hex_max = int(p["hex_max"], 16)
